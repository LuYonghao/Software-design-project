...
feature --constructor
	...
	make
		...
		do
			...
			loop
				...
				loop
					grid[row,column] := create {SECTOR}.make(row,column,create {EXPLORER}.make_explorer)
					column:= column + 1;
				end
				row := row + 1
			end
	...
	make_empty
		do
			create grid.make_filled (create {SECTOR}.make_dummy, 1, 1)
		end
...
feature --commands
	...
	create_stationary_item: ENTITY
		...
		do
			...
			inspect chance
			when 1 then
				create {YELLOW_DWARF}Result.make_yellow_dwarf
			when 2 then
				create {BLUE_GIANT}Result.make_blue_giant
			when 3 then
				create {WORMHOLE}Result.make_wormhole
			else
				create {YELLOW_DWARF}Result.make_yellow_dwarf -- create more yellow dwarfs this will never happen, but create by default
			end -- inspect
		end
